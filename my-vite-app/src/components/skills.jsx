...
const Skills = () => {
  ...
  return (
    <section id="skills" className="py-16 sm:py-20 bg-gradient-to-b from-[#192965] to-[#0A1235] relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        <motion.div 
          style={{ y: yBubble1 }}
          className="absolute top-20 right-[5%] w-32 sm:w-48 md:w-64 h-32 sm:h-48 md:h-64 rounded-full bg-[#D87EE8]/10 blur-2xl"
        />
        <motion.div 
          style={{ y: yBubble2 }}
          className="absolute top-[40%] left-[15%] w-40 sm:w-60 md:w-80 h-40 sm:h-60 md:h-80 rounded-full bg-[#7A93D8]/15 blur-3xl"
        />
        <motion.div 
          style={{ y: yBubble3 }}
          className="absolute bottom-[10%] right-[20%] w-36 sm:w-54 md:w-72 h-36 sm:h-54 md:h-72 rounded-full bg-[#FF9EFF]/10 blur-3xl"
        />
      </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10" ref={containerRef}>
          ...
          <div className="flex flex-wrap justify-center gap-2 px-4">
            {categories.map((category) => (
              <button
                ...
                className={`px-3 sm:px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 flex items-center gap-2 ${
                  activeTab === category.id
                    ? 'bg-[#FF9EFF] text-[#0A1235] shadow-lg'
        ...
        {/* Glowing skill orb - visual centerpiece - Hidden on mobile, shown on desktop */}
        <motion.div
          initial={{ opacity: 0, scale: 0.8 }}
          whileInView={{ opacity: 1, scale: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8 }}
          className="hidden md:block relative mx-auto mb-12 lg:mb-16 w-64 h-64 lg:w-80 lg:h-80"
        >
          <div className="absolute inset-0 rounded-full bg-gradient-to-br from-[#D87EE8]/30 to-[#7A93D8]/30 backdrop-blur-sm border border-white/10 flex items-center justify-center overflow-hidden shadow-[0_0_50px_rgba(216,126,232,0.3)]">
            ...
            <div className="absolute inset-[15%] rounded-full bg-[#FF9EFF]/5 backdrop-blur-md border border-white/5 flex items-center justify-center animate-pulse">
              <span className="text-white text-2xl lg:text-4xl font-bold">{activeTab[0].toUpperCase() + activeTab.slice(1)}</span>
            </div>
            
            {/* Orbiting skill categories */}
            {categories.map((category, index) => {
              const angle = (index * (2 * Math.PI)) / categories.length;
              const radius = window.innerWidth < 1024 ? 100 : 130;
              ...
              return (
                <motion.button
                  key={category.id}
                  onClick={() => setActiveTab(category.id)}
                  className={`absolute translate-x-[-50%] translate-y-[-50%] p-2 lg:p-3 rounded-full backdrop-blur-md border transition-all duration-300 ${
                    activeTab === category.id
                ...
                >
                  {React.cloneElement(category.icon, { size: window.innerWidth < 1024 ? 16 : 20 })}
                </motion.button>
              );
            })}
          </div>
        ...
        >
          {skillsByCategory[activeTab].map((skill, index) => (
            <motion.div
              ...
              className="bg-black/20 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/10 shadow-lg"
            >
              <div className="flex items-start gap-3 sm:gap-4 mb-3 sm:mb-4">
                <div className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-xl bg-[#D87EE8]/20 backdrop-blur-sm border border-[#D87EE8]/30 flex-shrink-0">
        ...
        <motion.div
          ...
          transition={{ duration: 0.6 }}
          className="relative mb-16 sm:mb-20 py-12 sm:py-16 px-4 sm:px-6 rounded-2xl overflow-hidden"
        >
          ...
          <div className="relative z-10">
            <h3 className="text-xl sm:text-2xl font-bold mb-6 sm:mb-8 text-white text-center">
              Additional <span className="text-[#FF9EFF]">Expertise</span>
            </h3>
            ...
            <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
              {[
                ...
                <motion.span 
                  ...
                  className="px-3 sm:px-4 py-2 bg-black/30 text-white rounded-full text-xs sm:text-sm font-medium border border-white/10 backdrop-blur-sm cursor-default"
                >
                  {skill}
                </motion.span>
              ))}
        ...
        >
          <div className="bg-black/20 backdrop-blur-sm rounded-2xl border border-white/10 p-6 sm:p-8 lg:p-10 text-center">
            <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 text-white">
              Currently <span className="text-[#FF9EFF]">Learning</span>
            </h3>
            <p className="text-white/80 text-sm sm:text-base mb-6 sm:mb-8 max-w-2xl mx-auto leading-relaxed">
            ...
            <div className="flex flex-wrap justify-center gap-3 sm:gap-4 mb-6 sm:mb-8 lg:mb-10">
              {[
                ...
                <motion.div 
                  ...
                  className="px-4 sm:px-6 py-2 sm:py-3 bg-[#D87EE8]/20 text-[#FF9EFF] rounded-lg text-xs sm:text-sm font-medium border border-[#D87EE8]/30 flex items-center shadow-lg shadow-[#D87EE8]/5"
                >
            ...
            <motion.button
              ...
              className="px-4 sm:px-6 py-2 sm:py-3 bg-[#FF9EFF]/10 hover:bg-[#FF9EFF]/20 border border-[#FF9EFF]/30 rounded-full text-[#FF9EFF] font-medium inline-flex items-center transition-all duration-300 text-sm sm:text-base"
            >
